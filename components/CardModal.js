...
const CARD_COLORS = [
  { key: '', color: '#e8e5e0', label: 'Keine', noColor: true },
  { key: 'peach', color: '#FFBE98', label: 'Peach' },
  { key: 'sage', color: '#9CAF88', label: 'Sage' },
  { key: 'rose', color: '#D4A5A5', label: 'Rose' },
  { key: 'ciel', color: '#7BBFCB', label: 'Ciel' },
  { key: 'apricot', color: '#E8A87C', label: 'Apricot' },
  { key: 'artgold', color: '#C9A96E', label: 'Gold' },
  { key: 'laurel', color: '#8FA68E', label: 'Laurel' },
  { key: 'flamingo', color: '#D4869B', label: 'Flamingo' },
  { key: 'cerulean', color: '#A8C5DA', label: 'Cerulean' },
  { key: 'mocha', color: '#A67B5B', label: 'Mocha' },
  { key: 'slate', color: '#6B7C93', label: 'Slate' },
]
...
export default function CardModal({ card, cols, staff, supabase, onClose, onUpdate, currentStaff, sendNotification }) {
  ...
  async function addComment() {
    ...
    const payload = {
      ...
      sender_init: currentStaff.init || '',
      sender_color: currentStaff.color || '#b8892a',
      sender_avatar: currentStaff.avatar_url || '',
      text,
      mentions: mentioned
    }
    const { error } = await supabase.from('card_comments').insert(payload)
    if (error) {
      console.error('card_comments insert failed:', error)
      alert('Kommentar konnte nicht gespeichert werden: ' + (error.message || 'Unbekannter Fehler'))
      return
    }
    setCommentText('')
    setCommentMentions([])
  ...
  async function toggleChecklist(item) {
    ...
    supabase.from('checklist_items').update({ done: !item.done }).eq('id', item.id)
  }
  ...
  async function deleteAttachment(att) {
    if (!confirm('Anhang wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.')) return
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 4, alignItems: 'center' }}>
                  {CARD_COLORS.map(col => (
                    <div key={col.key} onClick={() => save('card_color', col.key || null)}
                      title={col.label}
                      style={{ width: 18, height: 18, borderRadius: '50%', background: col.color||col.bg||'#eee', border: '1.5px solid transparent', boxShadow: (localCard.card_color||'')===col.key ? '0 0 0 2px #fff, 0 0 0 3.5px '+(col.color||col.bg) : 'none', cursor: 'pointer', transition: 'all .15s', position:'relative', flexShrink: 0 }}
        ...
        <div className="card-modal-scroll" style={{ flex: 1, overflowY: 'auto', padding: '14px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              ...
              {comments.map(c => {
                const author = staff.find(s => s.id === c.sender_id) || { init:c.sender_init || '?', name:c.sender_name || 'Unbekannt', color:c.sender_color || '#999', avatar_url:c.sender_avatar }
                return (
                  <div key={c.id} style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
                    ...
                    <div style={{ flex:1, background:'#f4f2ef', borderRadius:9, padding:'8px 10px' }}>
                      ...
                      <div style={{ fontSize:12, color:'#4a4540', lineHeight:1.45, whiteSpace:'pre-wrap' }}>{renderCommentText(c.text)}</div>
                    </div>
                  </div>
                )
              })}
              <div style={{ position:'relative', display:'flex', gap:8, alignItems:'flex-start' }}>
                <textarea value={commentText} placeholder='Kommentar schreiben... @Name' onChange={e => { const v=e.target.value; setCommentText(v); setCommentMentions(getMentionCandidates(v)); setCommentMentionIdx(0) }} onKeyDown={e => { if (commentMentions.length && (e.key==='ArrowDown' || e.key==='ArrowUp')) { e.preventDefault(); setCommentMentionIdx(i => e.key==='ArrowDown' ? (i+1)%commentMentions.length : (i-1+commentMentions.length)%commentMentions.length) } else if (commentMentions.length && (e.key==='Tab' || e.key==='Enter')) { e.preventDefault(); insertMention(commentMentions[commentMentionIdx]); return } else if (e.key==='Enter' && !e.shiftKey) { e.preventDefault(); addComment() } }}
