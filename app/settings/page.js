...
export default function Settings() {
  ...
  async function load() {
    ...
    setCats(s)
    // Load per-user appearance settings
    const { data: { session } } = await supabase.auth.getSession()
    if (session) {
      const { data: staffRow } = await supabase.from('staff').select('id').eq('email', session.user.email).single()
      if (staffRow) {
        const { data: us } = await supabase.from('user_settings').select('*').eq('staff_id', staffRow.id).single()
        if (us) {
  ...
  async function saveSetting(key, value) {
    const appearanceKeys = ['bg_color', 'font_size', 'card_size']
    if (appearanceKeys.includes(key)) {
      ...
      if (session) {
        const { data: staffRow } = await supabase.from('staff').select('id').eq('email', session.user.email).single()
        if (staffRow) {
          await supabase.from('user_settings').upsert({ staff_id: staffRow.id, [key]: value }, { onConflict: 'staff_id' })
        }
      }
    } else {
      // Global settings (categories, roles)
